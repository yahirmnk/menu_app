...
class MongoService {
  ...
  Future<List<Recipe>> getRecipesByDiet(String dietTag) async {
    final url = Uri.parse("$baseUrl/recipes?dietTag=$dietTag");
    ...
    if (response.statusCode == 200) {
      ...
      print("Error al obtener recetas: ${response.body}");
      return [];
    }
  }
  ...
  Future<List<Diet>> getDiets() async {
    final url = Uri.parse("$baseUrl/diets");
    final response = await http.get(url);
    ...
    if (response.statusCode == 200) {
      ...
      print("Error al obtener dietas: ${response.body}");
      return [];
    }
  }
}
